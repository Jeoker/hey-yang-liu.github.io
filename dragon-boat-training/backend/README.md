...
当前代码覆盖已部署并验收的 P0，以及正在联调的 P1 核心：建季、Form／响应 Sheet 绑定复核、初始化与成员导入、Form 提交触发器、十分钟共享名单缓存、默认训练模板、周草稿、整周确认或预约开放、加场草稿和单场发布。报名、候补、排座及归档仍不在当前实现范围。
...
根目录运行 `npm test` 会以 Apps Script 服务替身验证 P0 会话和 P1 赛季、绑定、同步、缓存与分层发布规则。P0 已完成真实 Google 与 Pages 验收；P1 的私有脚本升级及 `setupDragonBoatP1` 已执行，公开部署和端到端结果见[当前进度](../CURRENT-STATUS.md)，完成前不得把模拟测试写成线上通过。
