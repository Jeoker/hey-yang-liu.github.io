...
- P0、P1“建季并开放一周”、P2“报名与维护”及 P2.1 等待体验优化已完成对应真实验收。P3 后端、公开船位组件和 Coach 排座工作区已经实现并部署；核心真实 API、页面流程及双窗口版本冲突恢复通过，仍缺桌面拖动的独立证明，P3 暂不关闭。
...
本轮开始时第一场已有 `P2 Test Member 12` 的 Left 报名，因此没有按旧记录清空场次。本轮只新增 `P2 Test Member 01` 的 Ambient 报名并做私有排座，随后取消该报名。最新双端读回为：
...
1. **P3 只剩桌面拖动复核**：登录、真实双窗口版本冲突与恢复均已完成。原生自动化拖动已尝试池到船位、船位到船位，均未引起 DOM 变化；需人工浏览器复核才能区分工具限制与产品问题，不再把重复自动化尝试当作证据。窄屏点选已通过。人工测试继续使用隔离成员，只修改私有草稿、不发布，完成后恢复本轮改动。
2. **P1 剩余**：默认赛季切换、赛季日期调整、当周默认场次编辑／删除、预约开放界面与真实验收、训练修改／取消及完整结束状态。
3. **P4**：单场与整季私有归档、公开荣誉墙、冻结后更正说明及恢复流程；D3、D4 在归档配置实现前和用户明确，不能擅自补成决定。
4. **P5**：Safari、真实手机、长期配额／负载和交接。当前真实浏览器记录包括 Edge；390×844 视口不等于实体手机验收。没有在 Google 表格中刻意制造断电；本地故障注入不代表全部 P1 写入均完成真实中断测试。
