...
`ImportState` 记录源映射、去重标识、游标、失败项和分批进度。`Health` 汇总触发器、同步、归档扫描、成功时间、错误、部署版本、绑定及名单版本、缓存统计和历史待发布任务；缓存统计汇总写入，不逐次命中写表。`PublicHistoryIndex` 记录已校验训练快照及公开版本；`PublicHistorySeasons` 保存每季目录、训练摘要、更正投影和训练快照行号，供分页及单行读取。旧数据首次建立索引时允许一次全量迁移，日常公开读取不再扫描全部训练历史。
...
| 公开历史 | `historySeasons`、`seasonHistory`、`archivedPractice` | 已发布的历史目录与公开快照，不依赖默认赛季；目录和单季列表接受 `limit`／`cursor`，返回 `next_cursor`／`total_count` |
...
6. Coach 连续排座停止操作两秒后才提交完整草稿；两秒内的多次移动合并为一次写入。已有请求进行中时继续编辑不会阻塞界面，原请求确认后只为当前最新草稿安排一次后续保存。请求编号、版本冲突和未知结果恢复规则保持不变。
...
历史接口不依赖当前 `Members` 或默认赛季，使用独立缓存和版本。目录默认每页 30 条、最多 100 条并缓存五分钟；单季列表同样分页并缓存十五分钟；单场详情按索引行号读取并缓存一小时。缓存超过安全大小或写入失败时直接返回权威投影，不截断结果。更正说明写入后同步更新紧凑索引并失效对应缓存，原快照不改写。归档数据不再参与实时容量与名单计算。

管理审计默认每页 50 条、最多 100 条。赛季日志从尾部读取 `limit + 1` 行；系统日志按有限块倒序扫描并携带双游标，页面只在用户点击时继续加载。日志保留完整记录，分页只限制单次读取和响应，不删除审计证据。
...
部署账号须由队伍长期控制，记录重新授权、重新部署、触发器恢复和人工 Code 发放步骤。会话期限、扫描频率、归档工作量与时间预算和导入批次属于部署参数；上线时按官方配额验证，批量读写，避免逐单元格循环。定时任务只保证可恢复的最终完成，不承担精确报名截止。
