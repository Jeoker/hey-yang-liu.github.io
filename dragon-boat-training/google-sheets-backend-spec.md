...
产品规则和待定事项以 [项目说明](README.md) 为准；页面行为见 [前端规格](frontend-spec.md)。本文定义服务端实现约束；P0 会话与幂等写入已有本地自动化验证，真实 Google 集成及后续业务场景仍按部署章节验收。
...
显式无效或未公开赛季返回错误，未公开周排期及其训练也不能从公开详情接口读取；已开放周中尚未单独发布的新增草稿同样不可公开读取或报名。到赛季结束边界即停止公开 `members` 读取；`COMPLETED` 只提供已完成状态及既有训练的公开只读结果，归档后返回对应历史入口。响应按白名单构造，不能直接序列化整行、原始回答或内部文件定位。
